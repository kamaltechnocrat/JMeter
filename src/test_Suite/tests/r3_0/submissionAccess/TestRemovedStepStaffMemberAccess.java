...
public class TestRemovedStepStaffMemberAccess {
	...
	public void testRemovedSubStepStaffInApplicantSubmissionListNG() throws Exception {
		try {
			...
			Assert.assertFalse(ProjectUtil.openSubmissionInApplicantSubmissionList(foProj,IGeneralConst.gnrl_SubmissionA[0][0], "All"),"FAIL: Should not be able to Open Submission as Removed Sub-Step Staff Member");
	...
	public void testRemovedSubStepStaffInMyAssignedSubmissionListNG() throws Exception {
		try {
			...
			Assert.assertFalse(ProjectUtil.openSubmissionInMyAssignedSubmissionList(foProj,IGeneralConst.gnrl_SubmissionA[0][0], "Open Projects"),"FAIL: Should not be able to Open Submission as Removed Step Staff Member");
	 ...
	 */
	@Test(groups = { "ProjectNG" }, dependsOnMethods="removePOGMember")
	public void testRemovedSubStepSaffProjectCreationNG() throws Exception {
		try {
		
			GeneralUtil.switchToPOWithSubClerk(indx);
			
			Assert.assertFalse(foProj.createNewPOProjectOnly(true), "FAIL: Removed Sub-Step Staff Member Shouldn't be able to create new Project!");
	...
	public void testRemovedStepStaffInApplicantSubmissionListNG() throws Exception {
		try {
			...
			Assert.assertFalse(ProjectUtil.openSubmissionInApplicantSubmissionList(foProj,IGeneralConst.gnrl_SubmissionA[0][0], "All"),"FAIL: Should not be able to Open Submission as Removed Step Staff Member");
	 ...
	 */
	@Test(groups = { "ProjectNG" }, dependsOnMethods="removePOGMember")
	public void testRemovedStepSaffProjectCreationNG() throws Exception {
		try {
			
			GeneralUtil.switchToPOWithClerk(indx);
		
			Assert.assertFalse(foProj.createNewPOProjectOnly(true), "FAIL: Removed Step staff Member Shouldn't be able to create new Project!");
