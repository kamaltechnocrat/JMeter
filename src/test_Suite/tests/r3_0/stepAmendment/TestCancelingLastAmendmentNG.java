...
public class TestCancelingLastAmendmentNG {
	...
	public void createFOProjectNG() throws Exception {
		try {
			...
			foProj = new FOProject(fopp, "", true, 2, "-CancelLastAmend");
	...
	public void reSubmitStepAmendment() throws Exception {
		...
		try{
			...
			ProjectUtil.openEvaluateSubmissionFormletInList_New(foProj, IGeneralConst.approvQuoCritAuto[0][0], IFiltersConst.evaluateSubmissions_InProgress_StatusSubView, "No Propreties Fields LBF", false,"2", IFiltersConst.versionNumber_EqualTo);
	...
	public void cancelLastAmendment() throws Exception {
		
		try {
			
			GeneralUtil.switchToPOWithProjOfficer("1");
			
			Assert.assertFalse(AmendmentsUtil.cancelAmendedSubmission(foProj, approver3,IGeneralConst.approvQuoCritAuto[0][0],"1", IFiltersConst.versionNumber_EqualTo), "FAIL: The Token has been advanced, should not be able to Cancel Amended Submission");
