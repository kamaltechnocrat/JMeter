...
public class TestActiveAndfHideProjNG {
	...
	public void setUp() {
		...
		try {
			...
			Assert.assertTrue(FOPPUtil.changeFoppStatusAndHideProjects(fopp, IFoppConst.E3StatesFopp.active,hideHistoricalProj), "FAIL: error occur during the process of changing Funding State!");
	 ...
	 */
	@Test(groups = { "ProjectsNG" })
	public void canNotSubmitFOProjectNG() throws Exception {
		try {
			...
			Assert.assertFalse(FrontOfficeUtil.openFOSubmissionForm(foProj, IGeneralConst.gnrl_SubmissionA[0][0]), "FAIL: Should not Open Submission In FO when hide projects is on!");
