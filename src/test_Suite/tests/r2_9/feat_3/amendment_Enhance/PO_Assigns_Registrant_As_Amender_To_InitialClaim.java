...
public class PO_Assigns_Registrant_As_Amender_To_InitialClaim {
	...
	public void setUp() {
		try {
			...
			foProj = new FOProject(fopp,"Registrant-Amender-", true,1,EFormsUtil.createAnyRandomString(5));
	...
	public void requestAmendments() throws Exception {
		...
		String[] amendParams = AmendmentsUtil.initializeAmendmentParamsNoOptionalRex(foProj, "/qa_Registrant4/", IGeneralConst.initialClaim[0][0]);
	...
	public void testAmendedSubmissionsAccessableByRegistrant_1() throws Exception {
		...
		Assert.assertTrue(GeneralUtil.isTextFieldReadOnly(0),"FAIL: the eForm should in Read Only mode");
		
		ClicksUtil.clickLinks(IClicksConst.backToSubListLnk);		
	}
	
	@Test(groups = { "WorkflowNG" },dependsOnMethods="testAmendedSubmissionsAccessableByRegistrant_1")
	public void reSubmitAmendedSubmissions() throws Exception {
		
		GeneralUtil.switchToFOOnly();
		GeneralUtil.loginAnyFO("front4");		
		
		Assert.assertTrue(foProj.submitAmendedFOProject(IGeneralConst.initialClaim[0][0], true));
	}
