...
public class PO_ReAssign_Amendment {
	...
	public void setUp() {
		try {
			...
			proj = new Project(fopp,"Re-Assign-Amender-", true,true,EFormsUtil.createAnyRandomString(5));
	...
	@Test(groups = { "WorkflowNG" },dependsOnMethods="testAmendmentAvailableToOthers")
	public void testReAssignTheAmender() throws Exception {
		
		AmendmentsUtil.changeAmender(proj, IPreTestConst.Groups[6][1][3]);		
	}
	
	@Test(groups = { "WorkflowNG" },dependsOnMethods="testReAssignTheAmender")
	public void testReAssignAmender() throws Exception {
		
		GeneralUtil.switchToPOWithProjOfficer("5");
		
		Assert.assertTrue(AmendmentsUtil.isViewAmendmentSubmissionIconExists(proj,IGeneralConst.gnrl_SubmissionA[0][0]));
		
		Assert.assertTrue(AmendmentsUtil.isAmendmentSubmisstionInReadOnlyMode(proj, IGeneralConst.gnrl_SubmissionA[0][0]));
						
	}
	
	@Test(groups = { "WorkflowNG" },dependsOnMethods="testReAssignTheAmender")
	public void testAssignedAmender() throws Exception {
		
		GeneralUtil.switchToPOWithProjOfficer("4");
		
		Assert.assertTrue(AmendmentsUtil.isViewAmendmentSubmissionIconExists(proj,IGeneralConst.gnrl_SubmissionA[0][0]));
		
		Assert.assertFalse(AmendmentsUtil.isAmendmentSubmisstionInReadOnlyMode(proj, IGeneralConst.gnrl_SubmissionA[0][0]));
