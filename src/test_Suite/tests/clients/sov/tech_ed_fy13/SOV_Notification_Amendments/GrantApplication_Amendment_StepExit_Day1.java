...
public class GrantApplication_Amendment_StepExit_Day1 {
	...
	String propKey = "scenario_03";
	
	FundingOpportunity fopp;

	FOProject foProj;
	
	String jamesCulkeen ="JamesCulkeen";
	...
	public void setUp() throws Exception {
		...
		try {
			...
			// -----------------------------------
			sov = SovUtils.techEdFy13_Initialization(ESovFOPPs.TechEdFY13, ESovTechEdApplicantsNum.V009, "Initial Review", "TEE Final FN_3330");
	...
	public void verifyApplicationRequestNotif(int NotifIndex, int entryCount,
		...
		try {
			...
			foProj.setProjFullName("TEE SoWes FY13");
			
			
			Assert.assertTrue(NotificationsUtil.openNotificationLogInProject_New(
					foProj, ISovConst.sovTechEdStepsNames[0]),
	...
	@Test(groups = { "SovRegTest" }, dependsOnMethods = { "verifyApplicationRequestNotif" })
	public void testCaseTemplateNG() throws Exception {
		try {
			
			GeneralUtil.switchToFOOnly();
			
			GeneralUtil.loginAnyFO(jamesCulkeen);

			sov.setCurrStepName(ISovConst.sovTechEdStepsNames[0]);
			
			Assert.assertTrue(SovUtils.tedFY13_SubmitApplication(sov), "FAIL: Could not Submit Grant Application");
	...
	@Test(groups = { "SovRegTest" }, dependsOnMethods = { "testCaseTemplateNG" }, dataProvider = "NotifNameIndexStepExit", enabled = true)
	public void verifyApplicationSubmittedNotif(int NotifIndex, int entryCount,
			boolean resultExpected) throws Exception {
		try {
			GeneralUtil.switchToPOOnly();
			...
			Assert.assertTrue(NotificationsUtil.openNotificationLogInProject_New(
					foProj, ISovConst.sovCfpFy13StepsNames[0]),
					"FAIL: Could not open the notification log");
