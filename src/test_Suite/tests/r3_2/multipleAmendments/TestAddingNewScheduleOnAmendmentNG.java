...
public class TestAddingNewScheduleOnAmendmentNG {
	...
	public void setUp() {
		...
		try {
			...
			foProj = new FOProject(fopp, fopp.getFoppPreFix(), true, 3, "-CFG-AbleToRequestParentAmend-PA-Step");
	...
	public void reSubmitAwardStep() throws Exception {		
		try {
			...
			Assert.assertTrue(ProjectUtil.openPOAwardFormletInList(foProj, IProjectsConst.cfgAwardStep, "PA Submission Schedule"));
			
			foProj.addSchedules("CFG-Post-Award-Submission", 1, true, true, "CFGAdding", 1);
	
			Assert.assertTrue(FrontOfficeUtil.reSubmitCfgSubProject(foProj, IProjectsConst.EcfgReportsTypes.FINANCIAL), "FAIL: Could Not submit Sub Project");
