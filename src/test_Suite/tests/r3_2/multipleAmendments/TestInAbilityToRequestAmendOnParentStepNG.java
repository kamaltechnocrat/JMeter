...
public class TestInAbilityToRequestAmendOnParentStepNG {
	...
	public void setUp() {
		...
		try {
			...
			foProj = new FOProject(fopp, fopp.getFoppPreFix(), true, 3, "-CFG-UnableToRequestParentAmend-PA-Step");
	...
	public void completeWorkFlowToPaStep() throws Exception {
		try {
			...
			GeneralUtil.AppendToText("Closed");
	...
	public void reSubmitAwardStep() throws Exception {		
		try {
			...
			Assert.assertTrue(FrontOfficeUtil.reSubmitCfgSubProject(foProj, IProjectsConst.EcfgReportsTypes.FINANCIAL), "FAIL: Could Not submit");
	...
	public void verifyUnableRequestParentAmend() throws Exception {
		...
		try {
			...
			Assert.assertFalse(AmendmentsUtil.requestStepAmendmentWithSubProjectReExecute(foProj,amenders,dd,reason,comment,true, paStartSchedule));
