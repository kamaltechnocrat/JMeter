...
public class VerifySettingAndSavingGroupsInPaNG {
	...
	String[] grpsArr = {"G02 Reviewers","Staff","G06 Project Officers"};
	...
	public void selectingTheGroupsNG() throws Exception {
		try {
			...
			proj.initializeStep(IGeneralConst.reviewQuoCritManu[0][0].concat("-pa"));
			
			Assert.assertTrue(ProgramUtil.openStepDetails(ProgramUtil.findStepInProgPlanner(proj.getCurrentStepIdent(), 1)), "FAIL: could Open Step Details");
			
			Assert.assertTrue(FOPPUtil.selectEvaluationVisibilityGroups(grpsArr), "FAIL: could not select Groups");
			
			Assert.assertTrue(ProgramUtil.openStepDetails(ProgramUtil.findStepInProgPlanner(proj.getCurrentStepIdent(), 1)), "FAIL: could not Open Step Details");
