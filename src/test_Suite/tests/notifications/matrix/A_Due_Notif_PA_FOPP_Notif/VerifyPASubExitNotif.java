...
public class VerifyPASubExitNotif {
	// #####***********************************************************************
	// ### To set up the Global Params Name Vars
	// #####***********************************************************************
	Class<?> VerifyPASubExitNotif = this.getClass();

	private Log log = LogFactory.getLog(VerifyPASubExitNotif);

	FundingOpportunity fopp;

	FOProject foProj;
	
	String propKey = "scenario10";
	...
	public void setUp() throws Exception {
		...
		try {
			...
			foProj = new FOProject(fopp, "PA-Sub-Exit-", true, 1, "-10");
	...
	@Test(groups = { "NotificationWorkFlow" }, dependsOnMethods = { "submitInitialClaim" },dataProvider="NotifNameIndex", enabled = true)
	public void verifyNotifLogSubExitBeforeJobRun(int NotifIndex, int entryCount, boolean resultExpected) throws Exception {
		try {
	...
	@Test(groups = { "NotificationWorkFlow" }, dependsOnMethods = "verifyNotifLogSubExitBeforeJobRun", enabled = true)
	...
	public void verifyNotifLogSubExitAfterJobRun(int NotifIndex, int entryCount, boolean resultExpected) throws Exception {
		try {
