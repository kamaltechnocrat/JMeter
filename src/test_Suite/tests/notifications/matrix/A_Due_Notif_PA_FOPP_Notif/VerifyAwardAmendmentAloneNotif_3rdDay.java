...
public class VerifyAwardAmendmentAloneNotif_3rdDay {
	// #####***********************************************************************
	...
	@Test(groups = { "NotificationWorkFlow" }, dependsOnMethods = { "initializeProject" })
	public void resubmitAwardStep() throws Exception {
		try {
			
			GeneralUtil.switchToPOWithProjOfficer("1");
			
			Assert.assertTrue(foProj.reSubmitAnyStandardAgreement(IGeneralConst.gnrl_AwardCrit[0][0], true), "FAIL: Could not resubmit Award step!");
