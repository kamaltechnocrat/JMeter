...
/*
 * Test Case 03 (Projects List) For Story #1887. Search/Filter: sweep existing lists
...
public class S1887_03NG {
	//class fileds that will be used in the Filter Criteria

	//private static String projNumber[] = {"", ""};
	...
	@Test(groups = { "Iter_12" })
	public void s1887_3NG() throws Exception {
		...
		for(int j=0; j<IFiltersConst.filterModes.length; j++) {
			...
			for(int i=0; i<projName.length; i++){
				...
				if(TablesUtil.findInTable(ITablesConst.projectsTableId, projName[i]))
