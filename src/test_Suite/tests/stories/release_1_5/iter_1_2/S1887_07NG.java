...
/*
 * Test Case 07 (Evaluate Submission List) For Story #1887. Search/Filter: sweep existing lists
...
public class S1887_07NG {
	...
	@Test(groups = { "Iter_12" })
	public void s1887_7NG() throws Exception {
		...
		for(int j=0; j<IFiltersConst.filterModes.length; j++) {
			...
			for(int i=0; i<projName.length; i++){
				...
				fieldsValues.put(IFiltersConst.grantManagement_ProjectNumber_Lbl, projName[i].substring(startIndx, endIndx));
				
				modesValues.put(IFiltersConst.grantManagement_ProjectNumber_Lbl, IFiltersConst.filterModes[j]);				
				
				FiltersUtil.filterListByLabel(fieldsValues, modesValues, false);
				
				if(TablesUtil.findInTable(ITablesConst.fundingOpp_EvaluateSubmissionsTableId, projName[i]))
