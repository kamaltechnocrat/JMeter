...
/*
 * Test Case 04 (My In Basket List) For Story #1887. Search/Filter: sweep existing lists
...
public class S1887_04NG {
	...
	//private static String formName[] = {"Test Case 1 Fields", "Test Case 1 Fields"};
	...
	@Test(groups = { "Iter_12" })
	public void s1887_4NG() throws Exception {
		...
		for(int j=0; j<IFiltersConst.filterModes.length; j++) {
			...
			for(int i=0; i<projName.length; i++){
				...
				if(TablesUtil.findInTable(ITablesConst.fundingOpp_myAssignedSubmissionsTableId, projName[i]))
