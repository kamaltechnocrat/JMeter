 ...
 *  10.Evaluate PAP submission and Set to Rejected
 *  11 Make sure G3 Program Status is Inactive and Approval Process is set to Rejected
 *  
 */
...
public class S2614_02NG {

	private static Log log = LogFactory.getLog(S2614_02NG.class);
	IE ie;
	
	private String programId = IGeneralConst.baseLetters[0]+IGeneralConst.gnrl_ProgPrefix+
		IGeneralConst.gnrl_ProgName+"-"+IGeneralConst.baseLetters[1];
	private String papId = IGeneralConst.baseLetters[0]+IGeneralConst.gnrl_ProgPrefix+
	IGeneralConst.gnrl_PAPName+"-"+IGeneralConst.baseLetters[1];
	 ...
	 * Parent Method
	 */
	public void s2614_02NG() throws Exception
	{
	...
	private void evaluateSubmission()
	{
		try
		{
			...
			if(rowIndex > -1)
			{
				...
				ie.selectList(id, "/eFormFieldsBeans:0:numericDropdown/").select(IProgramsConst.APPR_STATUS_REJECTED);
				ie.textField(id, "/eFormFieldsBeans:1:datePicker/").set(GeneralUtil.getTodayDate());
				ie.textField(id, "/eFormFieldsBeans:2:textArea_Below/").set(IProgramsConst.APPR_STATUS_REJECTED);
				ClicksUtil.clickButtons(IClicksConst.saveBtn);
				try
				{
	...
	private void checkStatus3_4()
	{
		try
		{
			...
			if(ie.link(text, programId).exists())
			{
				...
				ErrorUtil.stringComparison(IProgramsConst.APPR_STATUS_REJECTED, 
						ie.label(id, IProgramsConst.fundingOpp_PROG_PAP_STATUS_LBL).text(), " PAP Status: ");
